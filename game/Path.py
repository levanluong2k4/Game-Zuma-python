...
class Path:
    ...
    def set_positions(self):
            # pos = Điểm bắt đầu (Vector2)
        pos = pygame.math.Vector2(self.targets[0])
        
        # direction = Vector2(0, 0)
        direction = pygame.math.Vector2((0, 0))

        # target_index = 0 (Bắt đầu với chỉ số mục tiêu đầu tiên)
        target_index = 0

        # Vòng lặp cho đến khi hết tất cả các mục tiêu
        while target_index < len(self.targets):
            # pos = pos + direction * step (Di chuyển đến vị trí tiếp theo)
            pos = pos + (direction * self.step)
            
            # Thêm vị trí vào danh sách positions
            self.positions.append(pos)

            # Nếu pos đạt đến vị trí mục tiêu hiện tại
            if (round(pos.x), round(pos.y)) == self.targets[target_index]:
                target_index += 1  # Tiến tới mục tiêu tiếp theo
                
                # Nếu đã đạt đến mục tiêu cuối cùng thì thoát khỏi vòng lặp
                if target_index == len(self.targets):
                    break
                
                # direction = Thay đổi hướng để di chuyển tới mục tiêu tiếp theo
                direction = self.change_direction(target_index, pos)


    def change_direction(self, target_index, pos):
            # direction = Vecto hướng từ pos đến target tại target_index
        direction = pygame.math.Vector2(
            (self.targets[target_index][0] - pos[0],
            self.targets[target_index][1] - pos[1]))
        
        # length = Tính độ dài của vector direction
        length = math.hypot(*direction)
        
        # direction = Chuẩn hóa vector (chia vector theo độ dài)
        direction = pygame.math.Vector2(
            (direction[0] / length,
            direction[1] / length))
    ...
    def draw(self, screen):
        # Lặp qua từng mục tiêu trong danh sách targets
        for i in range(len(self.targets) - 1):
            # Vẽ đường nối giữa target[i] và target[i+1] trên màn hình
